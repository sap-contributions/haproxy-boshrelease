...
describe 'config/haproxy.config rate limiting' do
  ...
  context 'when ha_proxy.connections_rate_limit properties "window_size", "table_size" are provided' do
    ...
    context 'when "connections", "block", "cidrs_to_exclude", and "http_error_rate_limit" are provided' do
      let(:properties) do
        temp_properties.deep_merge(
          { 'connections_rate_limit' =>
              {
                ...
                'block' => 'true',
                'cidrs_to_exclude' => ['10.0.0.0/8', '3.22.12.3/32'],
                'http_error_rate_limit' => '7'
              }
          })
      end

      it 'adds tcp-request connection reject with sc_conn_rate and sc_http_err_rate to http-in and https-in frontends' do
        expect(frontend_http).to include('acl cidr_list_to_exclude src -f /var/vcap/jobs/haproxy/config/cidrs_to_exclude_from_blocking.txt')
        expect(frontend_http).to include('tcp-request connection track-sc0 src table st_tcp_conn_rate')
        expect(frontend_http).to include('tcp-request connection reject if { sc_conn_rate(0) gt 5 } { sc_http_err_rate(0) gt 7 } !cidr_list_to_exclude')
        expect(frontend_http).to include('acl cidr_list_to_exclude src -f /var/vcap/jobs/haproxy/config/cidrs_to_exclude_from_blocking.txt')
        expect(frontend_http).to include('tcp-request connection track-sc0 src table st_tcp_conn_rate')
        expect(frontend_https).to include('tcp-request connection reject if { sc_conn_rate(0) gt 5 } { sc_http_err_rate(0) gt 7 } !cidr_list_to_exclude')
      end
    end
  end
end
